...
struct TrainingContext
{
    ...
    cudnnTensorDescriptor_t dataTensor, conv1Tensor, conv1BiasTensor, pool1Tensor,     //data
                             conv2Tensor, conv2BiasTensor, conv3Tensor, conv3BiasTensor,
                             pool2Tensor, fc1Tensor, fc2Tensor;
    cudnnFilterDescriptor_t conv1filterDesc, conv2filterDesc, conv3filterDesc;   //kernel
    ...
    TrainingContext(int gpuid, int batch_size,
                    ConvBiasLayer& conv1, MaxPoolLayer& pool1, ConvBiasLayer& conv2, MaxPoolLayer& pool2, ConvBiasLayer& conv3,
    ...
    void ForwardPropagation(float *data, float *conv1, float *pool1, float *conv2, float *pool2, float *conv3, float *fc1, float *fc1relu,
                            ...
                            float *pfc2, float *pfc2bias, void *workspace, float *onevec)
    {
        ...
        // FC1 layer
        // Forward propagate neurons using weights (fc1 = pfc1'*conv3)
        checkCudaErrors(cublasSgemm(cublasHandle, CUBLAS_OP_T, CUBLAS_OP_N,
                                    ref_fc1.outputs, m_batchSize, ref_fc1.inputs,
                                    &alpha,
                                    pfc1, ref_fc1.inputs,
                                    /*pool2*/conv3, ref_fc1.inputs,
                                    &beta,
                                    fc1, ref_fc1.outputs));
    ...
    void Backpropagation(ConvBiasLayer& layer_conv1, MaxPoolLayer& layer_pool1, ConvBiasLayer& layer_conv2, MaxPoolLayer& layer_pool2,ConvBiasLayer& layer_conv3,
                         float *data, float *labels, float *conv1, float *pool1, float *conv2, float *pool2, float *conv3, float *fc1, float *fc1relu,
                         ...
                         float *gconv3, float *gconv3bias, float *dconv3,
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    ConvBiasLayer conv2(conv1.out_channels, 30, 3, conv1.out_width / pool1.stride, conv1.out_height / pool1.stride);
    MaxPoolLayer pool2(2, 2);
    ConvBiasLayer conv3(conv2.out_channels, 70, 3, conv2.out_width / pool1.stride , conv2.out_height/pool2.stride );
    FullyConnectedLayer fc1((conv3.out_channels*conv3.out_width*conv3.out_height) , 500);
    //FullyConnectedLayer fc1((conv2.out_channels*conv2.out_width*conv2.out_height) , 500);
    FullyConnectedLayer fc2(fc1.outputs, 10);

    // Initialize CUDNN/CUBLAS training context
    TrainingContext context(FLAGS_gpu, FLAGS_batch_size, conv1, pool1, conv2, pool2, conv3, fc1, fc2);
    ...
    float *d_dpool1, *d_dpool2, *d_dconv2, *d_dconv3, *d_dfc1, *d_dfc1relu, *d_dfc2, *d_dfc2smax, *d_dlossdata;
    //                         Buffer     | Element       | N                   | C                  | H                                 | W
    //-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    checkCudaErrors(cudaMalloc(&d_dconv3,   sizeof(float) * context.m_batchSize * conv2.out_channels * conv2.out_height                  * conv2.out_width));
    ...
    for (int iter = 0; iter < FLAGS_iterations; ++iter)
    {
        ...
        // Forward propagation
        context.ForwardPropagation(d_data, d_conv1, d_pool1, d_conv2, d_pool2, d_conv3, d_fc1, d_fc1relu, d_fc2, d_fc2smax,
        ...
        context.Backpropagation(conv1, pool1, conv2, pool2, conv3,
                                d_data, d_labels, d_conv1, d_pool1, d_conv2, d_pool2, d_conv3, d_fc1, d_fc1relu, d_fc2, d_fc2smax, d_dlossdata,
                                d_pconv1, d_pconv1bias, d_pconv2, d_pconv2bias, d_pconv3, d_pconv3bias, d_pfc1, d_pfc1bias, d_pfc2, d_pfc2bias,
                                d_gconv1, d_gconv1bias, d_dpool1, d_gconv2, d_gconv2bias, d_gconv3, d_gconv3bias, d_dconv2, d_dpool2, d_dconv3, d_gfc1, d_gfc1bias,
    ...
    if (classifications > 0)
    {
        ...
        TrainingContext test_context(FLAGS_gpu, 1, conv1, pool1, conv2, pool2, conv3, fc1, fc2);
        ...
        for (int i = 0; i < classifications; ++i)
        {
            ...
            test_context.ForwardPropagation(d_data, d_conv1, d_pool1, d_conv2, d_pool2, d_conv3, d_fc1, d_fc1relu, d_fc2, d_fc2smax,
